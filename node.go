...
import (
	"bytes"
	"github.com/Kotodian/raft/utils"
	"log"
	"math/rand"
	"sync"
	"time"
...
func (r *RaftNode) Start(command interface{}) (index int, term int, isLeader bool) {
	index = -1
	term = -1
	isLeader = true
	if term, isLeader = r.GetState(); isLeader {
		r.mu.Lock()
		r.log = append(r.log, LogEntry{command, r.currentTerm})
		r.persist()
		r.matchIndex[r.me] = r.nextIndex[r.me] + len(r.log) - 1
		index = len(r.log) - 1
		DPrintf("[Start] raft %d replicate command to log | current term: %d | current state: %d | log length: %d\n",
			r.me, r.currentTerm, r.role, len(r.log))
		r.mu.Unlock()
	}
	return
}
